 ...
 * @param[in]  keyHandle Handle of a loaded key.
 * @param[in]  shandle1 First session handle.
 * @param[in]  shandle2 Second session handle.
 * @param[in]  shandle3 Third session handle.
 * @param[in]  label Optional label.
 * @param[out] outSymSeed The symmetric seed.
 *             (callee-allocated)
 * @param[out] cipherText The encapsulated cipher text.
 *             (callee-allocated)
...
Esys_Encapsulate(ESYS_CONTEXT           *esysContext,
    ...
    r = Esys_Encapsulate_Async(esysContext, keyHandle, shandle1, shandle2, shandle3, label);
     ...
     */
    do {
        r = Esys_Encapsulate_Finish(esysContext, outSymSeed, cipherText);
 ...
 *
 * This function invokes the TPM2_Encapsulate command in a asynchronous
 * variant. This means the function will return as soon as the command has been
 * sent downwards the stack to the TPM. All input parameters are const.
 * In order to retrieve the TPM's response call Esys_Encapsulate_Finish.
 *
 * @param[in,out] esysContext The ESYS_CONTEXT.
 * @param[in]  keyHandle Handle of a loaded key.
 * @param[in]  shandle1 First session handle.
...
Esys_Encapsulate_Async(ESYS_CONTEXT     *esysContext,
                       ...
                       const TPM2B_DATA *label) {
    TSS2_RC r;
    LOG_TRACE("context=%p, keyHandle=%" PRIx32 ", label=%p", esysContext, keyHandle, label);
 ...
 * invoked via Esys_Encapsulate_Finish. All non-simple output parameters
...
Esys_Encapsulate_Finish(ESYS_CONTEXT           *esysContext,
                        ...
                        TPM2B_ENCRYPTED_SECRET **cipherText) {
    TSS2_RC r;
    LOG_TRACE("context=%p, outSymSeed=%p, cipherText=%p", esysContext, outSymSeed, cipherText);
    ...
    if (outSymSeed != NULL) {
        *outSymSeed = calloc(1, sizeof(TPM2B_DATA));
        if (*outSymSeed == NULL) {
            return_error(TSS2_ESYS_RC_MEMORY, "Out of memory");
        }
    }
    if (cipherText != NULL) {
        *cipherText = calloc(1, sizeof(TPM2B_ENCRYPTED_SECRET));
        if (*cipherText == NULL) {
