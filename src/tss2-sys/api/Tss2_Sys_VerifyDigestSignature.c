...
TSS2_RC
Tss2_Sys_VerifyDigestSignature_Complete(TSS2_SYS_CONTEXT *sysContext, TPMT_TK_VERIFIED *validation) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
...
Tss2_Sys_VerifyDigestSignature(TSS2_SYS_CONTEXT             *sysContext,
    ...
    rval = Tss2_Sys_VerifyDigestSignature_Prepare(sysContext, keyHandle, digest, signature);
    if (rval)
        return rval;
